...
from utils.wandb import init_sweep


class RunRunner:

    def __init__(self, smiles_file="data/CPI/smiles.smi", storage_path="storage/CPI", 
                 latent_file="encoded_smiles.latent", decoded_smiles="decoded_smiles.csv", n_epochs=2000,
                 sample_n=30000, encoder=None):
        # init params
    ...
    def test(self):
        T = TrainModelRunner(input_data_path=self.output_latent, output_model_folder=self.storage_path,
                             decode_mols_save_path=self.decoded_smiles, n_epochs=self.n_epochs,
    ...
    def run(self, config=None):
        ...
        T = TrainModelRunner(input_data_path=self.output_latent, output_model_folder=self.storage_path,
                             decode_mols_save_path=self.decoded_smiles, n_epochs=self.n_epochs,
                             sample_after_training=self.sample_size, decoder=self.decoder, config=None)
        # T.run()
        print("Model finished.")
        T.test()


def sweep_run(runner, config=None):
    wandb.init()
    config = wandb.config
    print(config)
    runner.run(config)
...
if __name__ == "__main__":
    ...
    # sweep
    sweep_config = init_sweep()
    sweep_id = wandb.sweep(sweep_config, project="sweeps-demo")
    # wandb.init()
    wandb.agent(sweep_id, sweep_run(runner), count=50)

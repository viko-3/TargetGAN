...
class SmilesVectorizer(object):
    ...
    def __init__(self, charset='@C)(=cOn1S2/H[N]\\', pad=10, maxlength=120, leftpad=True, isomericSmiles=True,
                 augment=True, canonical=False, startchar='^', endchar='$', unknownchar='?', binary=False):
        # Special Characters
...
class ChemceptionVectorizer(object):
    ...
    def vectorize_mol(self, mol, augment=None):
        ...
        for i, bond in enumerate(mol.GetBonds()):
            ...
            bcoords = coords[bidx]
            ecoords = coords[eidx]
            frac = np.linspace(0, 1, int(
                1 / self.res * 2))  # with a res of 0.5 this should be adequate#TODO implement automatic determination/better line drawing algoritm.
            for f in frac:
...
class MorganDictVectorizer(object):
    ...
    def transform_mol(self, mol, misses=False, binary=False):
        """ transforms the mol into a dense array using the fitted keys as index
         ...
         """
        assert type(
            self.keys) is np.ndarray, "keys are not defined or is not an np.array, has the .fit(mols) function been used?"
